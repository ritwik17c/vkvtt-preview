...
function installControls(){if(document.getElementById('periodReminderControl'))return;const anchor=document.getElementById('myTimetableBtn')||document.querySelector('.myGrid');if(!anchor)return;const box=document.createElement('div');box.id='periodReminderControl';box.className='periodReminderControl';box.style.cssText='margin-top:10px;padding:12px;border:1px solid #d7c27a;border-radius:13px;background:#fffaf0';const p=prefs();box.innerHTML=`<b>🔔 Period Reminder</b><div style="font-size:.82rem;margin:4px 0 9px">Uses the <b>Activated Schedule</b>. Announces exactly 1 minute before your period while VKVTT is active.</div><label style="display:inline-flex;gap:6px;align-items:center;margin-right:12px"><input id="prEnabled" type="checkbox" ${p.enabled?'checked':''}> Enabled</label><label style="display:inline-flex;gap:6px;align-items:center;margin-right:12px"><input id="prVoice" type="checkbox" ${p.voice?'checked':''}> Voice</label><label style="display:inline-flex;gap:6px;align-items:center"><input id="prNotify" type="checkbox" ${p.notification?'checked':''}> Notification</label><button id="prTest" type="button" style="margin-left:10px">Test Voice</button><div id="prStatus" style="font-size:.78rem;margin-top:7px"></div>`;anchor.insertAdjacentElement('afterend',box);
